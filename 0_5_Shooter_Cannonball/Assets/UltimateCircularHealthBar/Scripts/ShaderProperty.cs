...
namespace RengeGames.HealthBars {
	...
	public class ShaderProperty<T> : IShaderProperty {
		...
		public ShaderProperty(string propertyName, Func<int, bool, T, T> propertyFunc, T value = default) {
			...
			//special case for gradients and animation curves
			if (value is Gradient g) {
				_value = (T) (object) g.Clone();
			}else if (value is AnimationCurve c) {
				_value = (T) (object) c.Clone();
			}
			else {
				_value = value;
			}
		...
		public void ApplyValueToShader(bool ignoreDirty, ref T value) {
			...
			if (value is AnimationCurve c) {
				ApplyCurveValueToShader(ignoreDirty, ref c);
				return;
			}
			if (IsDirty && !ignoreDirty) {
				value = _value;
		...
		private void ApplyCurveValueToShader(bool ignoreDirty, ref AnimationCurve value) {
			if (value == null || !(_value is AnimationCurve)) return;
			if (IsDirty && !ignoreDirty) {
				value = ((AnimationCurve) (object) _value).Clone();
				_propertyFunc(_propertyId, true, _value);
				IsDirty = false;
			}else if (ignoreDirty || !value.EqualTo((AnimationCurve) (object) _value)) {
				_value = (T) (object) value.Clone();
				_propertyFunc(_propertyId, true, _value);
				IsDirty = false;
			}
